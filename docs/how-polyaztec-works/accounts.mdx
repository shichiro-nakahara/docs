...
Accounts in PolyAztec. 

For a deep dive into the technical details of PolyAztec accounts, see the [Account Circuit specification](../spec/account_circuit).

## A Technical Primer on Accounts

There are two main parts to each PolyAztec account, the account (private/public key pair) and signers. The account is associated with a [viewing key](../glossary.md#viewing-key) that can be used to decrypt [value notes](../glossary#value-notes) (assets on PolyAztec). The signers are associated with [spending keys](../glossary.md#spending-key) (or signing keys) which can be used to send notes from the associated account.
...
PolyAztec abstracts away much of this complexity to make it easier for users to follow best practices. For example, when you look up an account by alias (meaning the account has been registered) a transfer automatically marks the notes as spendable by the recipients spending keys. You can still choose to mark value notes sent to an account as spendable by the account private key, but this is not recommended because the recipient may have shared that key with an other parties in order to allow it to decrypt value notes to calculate balances.

### Compared to Polygon

Accounts in PolyAztec work differently than accounts in Polygon.

PolyAztec uses a different curve than Polygon for SNARK efficient operations. This means that you cannot use an Polygon private key directly for signing PolyAztec transactions or a public key for deriving an account address. Specifically, PolyAztec uses the Grumpkin curve, see [the yellow paper](https://hackmd.io/@aztec-network/ByzgNxBfd#2-Grumpkin---A-curve-on-top-of-BN-254-for-SNARK-efficient-group-operations) for more information.

In [PolyAztec](https://polyaztec.xyz), PolyAztec accounts are generated using Polygon accounts by having a user sign a message and deriving the PolyAztec keys from the signed message. This ensures that as long as someone has access to their Polygon account, they will be able to access their PolyAztec account by signing a message. Different messages are used to generate different keys (account decryption key and spending key).

## Users And Accounts

Users in PolyAztec will use the basic account to receive notes (via their public key) and decrypt balances (with the viewing key) and one of their registered spending keys to spend notes and initiate bridged Polygon interactions.

### Account

A basic account is the first account that is generated for an PolyAztec user--it is a simple private/public key pair.
...
PolyAztec tries to abstract much of this complexity away and presents everything to the user as if this notion does not exist (e.g. the account balance is the sum of all notes regardless of registered or not).

When actually creating the zero knowledge proof, PolyAztec infers which balance you're drawing from based on whether you give it a spending key or the account key.
...
### What happens if I lose my PolyAztec account private key?

Your PolyAztec account private keys are derived from an Polygon signature we ask you to sign when you register with us. As long as you still control your original Polygon account you can re-derive your PolyAztec account keys.

---

### What happens if I lose my PolyAztec account private key AND my Polygon account private key?

At the current time, your funds would be lost. Our protocol architecture supports PolyAztec account social recovery but implementation into our front-end software is still under development.

---

### What is the PolyAztec username for?

PolyAztec username/alias lets other users easily lookup your encryption public key so they can send you assets. This name has to be unique and is limited to 20 characters, lowercase, alphanumeric. Please note that this isn’t an ENS domain, but it serves a similar purpose within the confines of our network.

---

### I've registered to the platform, but PolyAztec prompts registration once again when I try to log in again?

Please follow these instructions:

**Step 1:** Clear browser cache. For Chrome, this is the link: chrome://settings/cookies/detail?site=polyaztec.xyz
