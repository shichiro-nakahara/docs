...
__Fully Confidential Polygon Transactions: PolyAztec's Privacy Architecture__

_An overview of how PolyAztec preserves privacy._

[PolyAztec](https://PolyAztec.network/) is a privacy-first zero-knowledge rollup on Polygon: that means it's the only [Layer 2](https://Polygon.org/en/developers/docs/scaling/layer-2-rollups/) built from the ground up to be fully privacy preserving.
...
When an PolyAztec transaction processes, rather than doing an account balance update (incrementing and decrementing balance), the network simply re-assigns ownership for a given note.
...
To ensure nothing fishy happens in step 1, all Snoop needs to do is prove to the system (PolyAztec) that the two notes he intends to create are equivalent in value to the two notes he intends to destroy.
...
PolyAztec then validates the proof and says, “By the powers of Zero Knowledge, this must be true,” at which point the smart contract destroys the two input notes, generates the two output notes, and records the new output notes as an encrypted commitment in the note registry.

### Proving Ownership

It's worth discussing how ownership of notes is proven in PolyAztec, which has analogies to Polygon-world. How do you prove you control access to an address in Polygon? You sign a message using your wallet.

How do you prove you control access to a note in PolyAztec? With a very very fancy cryptographic signature called a zero knowledge proof.

The proof says, “Somewhere in PolyAztec's state, there: a) exists a note with a certain value, and b) I own it.”

And how's the state of the PolyAztec system stored? In two [Merkle Trees](https://en.wikipedia.org/wiki/Merkle_tree):
...
Saying you own a note indicates to PolyAztec that the note exists in the note tree, and that no corresponding note-nullifier exists in the nullifier tree.
...
These unique constraints mean privacy-preserving architecture must be constructed from the ground up. PolyAztec is the only L2 built this way — with privacy protected by its core architecture and facilitated by the magic of zero knowledge. *Grazie mille!*
...
Transactions within the PolyAztec rollup are hidden from observers and the PolyAztec rollup provider. Both the identities of sender/recipient and the values transferred are encrypted.
...
When you deposit into our rollup, people can see the Polygon address you deposited from, and how much you deposited. They cannot see the identity of the PolyAztec address you deposited into.
...
**For privacy, you need to withdraw to a different Polygon address.** Use a different address than the one you fund or register PolyAztec username with.

---

### What information about my transaction will be shown on explorer like Polygonscan?

It will show the funds were sent from an “PolyAztec Contract / Private Rollup”.
